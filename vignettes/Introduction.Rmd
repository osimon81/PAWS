...
lp_manual_stim <- read.csv(system.file("extdata", "Example_6J_female_lp_manual.csv", package = "PAWS"))
lp_arm_stim <- read.csv(system.file("extdata", "Example_6J_female_lp_arm.csv", package = "PAWS"))
```
...
PAWS runs analysis primarily on a univariate projection — a combination of the x and y components, weighted based off of how much movement there is in a particular axis. It's important to not only graph the raw trajectories of a paw over time, but also to graph the univariate projection and other diagnostic plots built into paws. To produce these plots using the settings from the most recent smoothing settings, you can run the following function:

```{r plotting smoothed univariate projection with average filter}
...
```{r plotting raw univariate projection}
plot_univariate_projection(csv_or_path = lp_arm_stim,
                   p_cutoff = 0,
                   reference_distance = 40,
                   manual_scale_factor = NA,
